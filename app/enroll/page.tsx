// components/EnrollmentForm.tsx - SANITIZED + PRICING 2026
"use client"
...
export default function EnrollmentForm() {
  ...
  const WHATSAPP = "27647919022"; // 0647919022 -> 27 format

  const [form, setForm] = useState({
    childFirstName: "",
    childAge: "",
    childGroup: "", // 0-2 or 2-5
    parentFirstName: "",
    parentPhone: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      fees: {
        registration: "R100 once-off",
        monthly: form.childGroup.includes("0-2")? "R280" : "R250"
      },
      consentPOPIA: form.consentPOPIA,
      timestamp: new Date().toISOString()
    };

    // Build WhatsApp message - Sanitized
    ...
    // 2. Open WhatsApp to 0647919022 with pre-filled
    ...
    <section className="bg-white">
      ...
      <div className="max-w-3xl mx-auto px-6">
        <div className="flex items-center gap-2">
          <Lock size={20} className="text-[#0F2C5C]"/><h2 className="font-bold text-[#0F2C5C] text-xl">Enrollment  - NO785 Radium</h2>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
          <div className="bg-[#0F2C5C] text-white p-3 rounded-xl">
            <Wallet size={16} className="mx-auto"/><p className="text-xs mt-1 opacity-70">Registration</p><p className="font-bold text-lg">R100</p><p className="text-[10px] opacity-70">Once-off</p>
          </div>
          <div className="bg-[#FFF8E1] border-2 border-[#FFD93D] p-3 rounded-xl">
            <Baby size={16} className="mx-auto text-[#0F2C5C]"/><p className="text-xs mt-1">0-2 Years</p><p className="font-bold text-lg text-[#0F2C5C]">R280</p><p className="text-[10px]">per month</p>
          </div>
          <div className="bg-[#EAF1F8] border p-3 rounded-xl">
            <Baby size={16} className="mx-auto text-[#0F2C5C]"/><p className="text-xs mt-1">2-5 Years</p><p className="font-bold text-lg text-[#0F2C5C]">R250</p><p className="text-[10px]">per month</p>
          </div>
        </div>

        <div className="mt-4 bg-yellow-50 border border-yellow-400 p-3 rounded-lg text-xs flex gap-2">
          <AlertTriangle size={14} className="text-yellow-700 mt-0.5"/>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4 bg-gray-50 p-6 rounded-xl border">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.parentPhone} onChange={e=>setForm({...form, parentPhone:e.target.value.replace(/[^0-9]/g,'').slice(0,10)})} placeholder="0647919022 or your number" className="mt-1 w-full border px-3 py-2.5 rounded-lg text-sm" required/>
            </div>
          </div>
          ...
          <div className="bg-white border p-3 rounded-lg text-xs space-y-2">
            <label className="flex gap-2"><input type="checkbox" checked={form.consentPOPIA} onChange={e=>setForm({...form, consentPOPIA:e.target.checked})} required/><span><b>POPIA *:</b> I consent NPC 2026/447870/08 storing first name + phone for enrollment only. Delete via compliance@...</span></label>
          ...
          {/* FEE SUMMARY */}
          <div className="bg-[#0F2C5C] text-white p-3 rounded-lg text-sm flex justify-between">
            ...
            <div className="text-right"><p className="text-[11px] opacity-70">We reply with EFT request</p><p className="font-bold text-[#FFD93D]">Capitec</p></div>
          </div>
          ...
          {status && <p className="text-xs text-center p-3 bg-white border rounded-lg font-bold">{status}</p>}
        </form>

        <div className="mt-4 mb-4 text-center text-[11px] text-gray-500">
          <p>🔐 Sanitized: No HTML, allowlist only, 30 char max, no ID, honeypot, rate limit backend. WhatsApp request encrypted.</p>
          <p>NPC 2026/447870/08 | NO785 | Enquiries: 0647919022 | <a href="mailto:tsundzukaniecd@gmail.com?subject=Enquiry%20via20Tsundzukani%Website" className="text-blue hover:text-[#FFD93D]">compliance@tsundzukanicentre.co.za</a></p>
        </div>
      </div>
      <Footer/>
    </section>
